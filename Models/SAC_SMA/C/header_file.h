...
#define MAXDAY 4506 /*15000*/  /* Maximum number of days in simulation */
#define MAXTSTEP 18024 /*4081 13214 */ /* 4081 */ /* Maximum numberoftimesteps */
...
struct SMA { /* I/O passed in function FLAND1 (THE SMA
  ...
  double tci_total[MAXTSTEP*TIMESPERDAY],tci_in[MAXTSTEP*TIMESPERDAY]; 
  double UH[19],UH_nord,tci_out[MAXTSTEP],tlci_flows[7];  
};
...
struct OBS {
     ...
     double Qobs [MAXTSTEP] [MAXGAGE];
};

struct OUTPUT {
  int numcomponents;
  int message[MAXTSTEP];
  double Qcomp_total [MAXTSTEP] [MAXGAGE];
  double Qcomp [MAXTSTEP] [MAXFLOWS];
  double states [MAXTSTEP] [MAXFLOWS];
  double excess [MAXTSTEP] [MAXFLOWS];
  double ET [MAXTSTEP] [MAXFLOWS];
};
