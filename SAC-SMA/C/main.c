...
/*     FILE *fopen(); */
     FILE *indata,*inpars,*inopti,*inobject,*outflux,
	  *outstats,*out3,*out4 ;
     int i,in,j,k,flag_total;
     char s[300],s1[100],s2[100],s3[100];
     char in1[100],in2[100],in3[100],in4[100];
     char calflag_char[20],name[20],*ps;
     float floatdummy;
     double LB,UB,CU;

     struct PARAMETERS parameters;
     struct PARAMETER parameter[MAXPAR];
     struct CRITERION criterion[MAXSTATS];
...
/*     printf("\nNAME YOUR 2 COLUMN OUTPUT FILE FOR FLUXES: "); */
     ...
     if(inpars==NULL) { 
	  printf("\n File %s is not opened!",inpars); 
	  return 0;
     } 
     else {

/*       printf("\n\n\n Opened the model parameters file %s:",inpars); */
     ...
     while (fgets(s,MAXLINE,indata) != NULL) { 
	  sscanf(s,"%lf %lf %lf %lf %lf %lf %lf %lf %lf",&floatdummy,
		 &floatdummy,&floatdummy,&obs.Qobs[i][0],&obs.PET[i][0], 
		 &obs.Precip[i][0],&obs.Precip[i][1],&obs.Precip[i][2],  
		 &obs.Precip[i][3]);  
	  obs.Qobs[i][0] = obs.Qobs[i][0]; /* what this line is DOING????? */
	  i++; 
     } 
     obs.datalength = i;
...
sac_sma(&parameters,&obs,&output); /* CALL to sac_sma */
