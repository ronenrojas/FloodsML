...
def get_item_method_check():
    ...
    for i in range(len(ds_train_old)):
        ...
        for j in range(ds_train_old.seq_length):
            ...
            if indices.numel() > 0:
                print("in sample number: {}, in {} in the sequence there is difference in X".format(i, j))
            abs_y1_y2 = np.abs(y1 - y2)
            y = np.argwhere(abs_y1_y2 > 0.0001)
            if y.numel() > 0:
                print("in sample number: {}, in {} in the sequence there is difference in Y".format(i, j))
            print("The number of not equal items is: {}".format(y.size()))
            print("The biggest difference is: {}".format(torch.max(abs_y1_y2)))
            print("The sum of differences is: {}".format(abs_y1_y2.sum()))
